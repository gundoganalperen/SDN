...
TABLE_ROUTING = 0
FLOW_DEFAULT_IDLE_TIMEOUT = 100             #   Idle Timeout value for flows
CONGESTED_TO_UNCON_TIME = 3                 #   If the congested links become non-congested, homer returns back to original shortest path again.
MAX_BW = 12500000                           #   Maximum throughput capacity on the link
ENABLE_QoS = True                           #   Set QoS properties with boolean expression.
IS_CONGESTED_PERIOD = 0.2                   #   Congestion check period to check the each link on the Homer path.
...
class QoS(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.homer_to_controller = []        #   Homer to Homer Controller traffic
        self.controller_to_homer = []        #   Homer Controller to Homer traffic
        self.new_path_added = False          #   Flag not to install the same flow multiple times
    ...
    # Create another hub to controller the homer path periodically(e.g. 0.5s)
            ...
            #calculate the number of hops on homer path
                ...
                for link in range(len(homer_path) - 1):
                    if self.graph[homer_path[link]][homer_path[link + 1]]['congested'] and not self.new_path_added:
                        ...
                        # Install the new flows into the switches
    ...
    def add_to_homer_path(self, path, flag):
        ...
        :return:
        """
        if flag == 0: #     add hops to Homer_to_controller array
        ...
        if flag == 1: #     add hops to Controller_to_homer array
            for hop in path:  #     The switches between the incoming switch and the server
    ...
    def add_QoS_path(self, path):
        ...
        if path == self.homer_to_controller:
            new_path = self.calculate_path_to_server(path[0], HOMER_CONTROLLER_MAC, balanced=True)
            self.logger.info("Previous path %s : HOMER Calculated QoS path between homer_to_controller: %s" % (path, new_path))
        ...
        # Calculate the new path controller to Homer
            ...
            new_path = self.calculate_path_to_server(path[0], HOMER_MAC, balanced=True)
            self.logger.info("Previous path %s HOMER Calculated QoS path between controller to homer: %s" % (path, new_path))
    ...
    def _print_topo(self):
        ...
        Plot the current network nodes.
    ...
    def _handle_ipv4(self, datapath, in_port, pkt):
        ...
        if (net_src == HOMER_IP and net_dst == HOMER_CONTROLLER_IP) or (net_src == HOMER_CONTROLLER_IP and net_dst == HOMER_IP):
            ...
            if net_src == HOMER_IP and net_dst == HOMER_CONTROLLER_IP:  # address of homer.
    ...
    def _handle_simple_switch(self, datapath, in_port, pkt, buffer_id=None, eth_dst=None):
        ...
        if eth_dst in self.mac_to_port[dpid]:
            ...
            if dl_src == 'b8:27:eb:2b:3a:41':
                tmp = 'b8:27:eb:b9:d8:7a'
        else:
